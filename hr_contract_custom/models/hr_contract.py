...
class Contract(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(Contract, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,
                                                    submenu=submenu)
        if view_type == "form" and self.env.user.has_group('security_groups.group_hr_employee'):
    ...
    def _onchange_manager_id(self):
        for rec in self:
            rec = rec.sudo()
            appraisals = self.env['hr.appraisal'].search([('employee_id', '=', rec.employee_id.id)])
