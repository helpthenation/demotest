...
class ApprovalRequest(models.Model):
    ...
    related_company_employee_id = fields.Char(related='related_employee_id.company_employee_id')
    ...
    company_employee_id = fields.Char('Employee Company ID')
        ...
        if view_type == "form":
            ...
            for field in res['fields']:
                for node in doc.xpath("//field[@name='%s']" % field):
                    print(field)
                    if field == 'approver_ids' and self.env.user.has_group('security_groups.group_company_employee'):
                        print('in fct')
                        node.set("readonly", "1")
    ...
    def _get_user_group(self):
        ...
        for rec in self:
            ...
            if user.has_group('security_groups.group_company_employee'):
    ...
    def onchange_employee_transferred(self):
        ...
        if self.employee_transferred and self.category_id.is_internal_transfer_request == 'yes':
            self.company_employee_id = self.employee_transferred.company_employee_id
    ...
    def create(self, vals):
        ...
        elif res.is_requisition_request == 'yes' and self.env.user.has_group('security_groups.group_company_employee'):
            group = res.group
    ...
    def write(self, vals):
        ...
        if vals.get(
            ...
            self.section = self.related_employee_id.contract_id.section
            self.subsection = self.related_employee_id.contract_id.subsection
        elif self.is_requisition_request == 'yes' and self.env.user.has_group('security_groups.group_company_employee'):
        ...
        for request in self:
            if request.is_requisition_request == 'yes':
                ...
                if request.employee_transferred:
                    request.name = request.category_id.name + " - " + request.employee_transferred.name + " - " + request.employee_transferred.company_employee_id
                    continue
    ...
    def _onchange_category_id(self):
        ...
        elif self.category_id.is_manager_approver:
        ...
        # if self.category_id.is_requisition_request:
        #     employee = self.direct_employee_id
        #     department_code = employee.department_id.type
        #     if department_code == 'BU':
        #         groups = self.env['hr.department'].search([('id', '=', employee.department_id.id)])
        #         # departments = self.env['hr.department'].search([('id', 'in', (employee.department_id.children_ids))])
        #         # groups = self.env['hr.department'].search([('id', 'in', (employee.department_id.id))])
        #         # groups = self.env['hr.department'].search([('id', 'in', (employee.department_id.id))])
        #         domain = {'group': [('id', 'in', groups.ids)]}
        #     elif department_code == 'BD':
        #         groups = self.env['hr.department'].search([('id', '=', employee.department_id.parent_id.id)])
        #         departments = self.env['hr.department'].search([('id', '=', employee.department_id.id)])
        #         # groups = self.env['hr.department'].search([('id', 'in', (employee.department_id.id))])
        #         # groups = self.env['hr.department'].search([('id', 'in', (employee.department_id.id))])
        #         domain = {'group': [('id', 'in', groups.ids)], 'department': [('id', 'in', departments.ids)]}
        #     elif department_code == 'BS':
        #         groups = self.env['hr.department'].search([('id', '=', employee.department_id.parent_id.parent_id.id)])
        #         departments = self.env['hr.department'].search([('id', '=', employee.department_id.parent_id.id)])
        #         sections = self.env['hr.department'].search([('id', '=', employee.department_id.id)])
        #         # subsections = self.env['hr.department'].search([('id', 'in', (employee.department_id.id))])
        #         domain = {'group': [('id', 'in', groups.ids)], 'department': [('id', 'in', departments.ids)],
        #                   'section': [('id', 'in', sections.ids)]}
        #     elif department_code == 'SS':
        #         groups = self.env['hr.department'].search(
        #             [('id', '=', employee.department_id.parent_id.parent_id.parent_id.id)])
        #         departments = self.env['hr.department'].search(
        #             [('id', '=', employee.department_id.parent_id.parent_id.id)])
        #         sections = self.env['hr.department'].search([('id', '=', employee.department_id.parent_id.id)])
        #         subsections = self.env['hr.department'].search([('id', '=', employee.department_id.id)])
        #         domain = {'group': [('id', 'in', groups.ids)], 'department': [('id', 'in', departments.ids)],
        #                   'section': [('id', 'in', sections.ids)], 'subsection': [('id', 'in', subsections.ids)]}
        #
    ...
    def _on_group_destination_change(self):
        if self.group_destination:
            # if self.department_destination.parent_id.id != self.group_destination.id:
            # self.department_destination = False
            # if self.section_destination.parent_id.parent_id.id != self.group_destination.id:
            # self.section_destination = False
            # if self.subsection_destination.parent_id.parent_id.parent_id.id != self.group_destination.id:
    ...
    def _on_department_destination_change(self):
        if self.department_destination:
            # if self.section_destination.parent_id.id != self.department_destination.id:
            # self.section_destination = False
            # if self.subsection_destination.parent_id.parent_id.id != self.department_destination.id:
            # self.subsection_destination = False
            return {'domain': {
                'section_destination': [('type', '=', 'BS'),
                                        ('parent_id', 'child_of',
    ...
    #     event_type = self.env['sap.event.type'].search([('name', 'ilike', '%XO%')])
    #
    #     source_dep = self.subsection_source if self.subsection_source.id else (
    #         self.section_source if self.section_source.id else (
    #             self.department_source if self.department_source.id else (
    ...
    #             'name': event_type.id,
    #             'event_reason': False,
    #             'start_date': self.transfer_from_date,
    #             'end_date': False,
    #             'employee_id': self.employee_transferred.id,
    #             'is_processed': False,
    #             'is_triggered': False,
    #             'related_requisition': self.id,
    #             # 'company_employee_id': event_type.id,
    #             # 'org_unit': event_type.id,
    #             # 'org_unit_fkey': event_type.id,
    #             # 'line_manager_id': event_type.id,
    #             # 'line_manager_id_fkey': event_type.id,
    #             # 'position_code': event_type.id,
    #             # 'position_code_fkey': event_type.id,
    #             # 'cost_center': event_type.id,
    #             # 'cost_center_fkey': event_type.id,
    #             # 'employee_group': event_type.id,
    #             # 'employee_group_fkey': event_type.id,
    #             # 'employee_sub_group': event_type.id,
    #             # 'employee_sub_group_fkey': event_type.id,
    #             # 'payroll_area': event_type.id,
    #             # 'payroll_area_fkey': event_type.id,
    #             # 'contract_type': event_type.id,
    #             # 'contract_type_fkey': event_type.id,
    #             # 'probation_end_date': event_type.id,
    #             # 'confirmation_date': event_type.id,
    #             # 'salutation': event_type.id,
    #             # 'salutation_fkey': event_type.id,
    #             # 'first_name': event_type.id,
    #             # 'middle_name': event_type.id,
    #             # 'last_name': event_type.id,
    #             # 'birth_date': event_type.id,
    #             # 'gender': event_type.id,
    #             # 'gender_fkey': event_type.id,
    #             # 'nationality': event_type.id,
    #             # 'nationality_fkey': event_type.id,
    #             # 'birth_country': event_type.id,
    #             # 'birth_country_fkey': event_type.id,
    #             # 'shift_type': event_type.id,
    #             # 'shift_type_fkey': event_type.id,
    #             # 'ot_eligibility': event_type.id,
    #             # 'system_id': event_type.id,
    #             # 'email_id': event_type.id,
    #             # 'payscale_group': event_type.id,
    #             # 'payscale_group_fkey': event_type.id,
    #             # 'payscale_level': event_type.id,
    #             # 'payscale_level_fkey': event_type.id,
    #             # 'related_compensations': event_type.id,
    #             # 'related_inactive_compensations': event_type.id,
    #             # 'related_compensation': event_type.id,
    #             # 'inactive_related_compensation': event_type.id,
    #             'is_esd': False
    #         })
    #         event.onchange_employee()
    #         event.org_unit_fkey = destination_dep.id
    #     elif self.transfer_type == 'temporary':
    #         event1 = self.env['hr.employee.event'].create({
    #             'name': event_type.id,
    #             'event_reason': False,
    #             'start_date': self.transfer_from_date,
    #             'end_date': self.transfer_to_date,
    #             'employee_id': self.employee_transferred.id,
    #             'is_processed': False,
    #             'is_triggered': False,
    #             'related_requisition': self.id,
    ...
    #             'name': event_type.id,
    #             'event_reason': False,
    #             'start_date': self.transfer_to_date + datetime.timedelta(days=1),
    #             'end_date': False,
    #             'employee_id': self.employee_transferred.id,
    #             'is_processed': False,
    #             'is_triggered': False,
    ...
    has_company_employee_id = fields.Selection(related="category_id.has_company_employee_id")
    ...
    def _run_always(self):
        for rec in self:
            rec.can_see_eligible_not_stored = self._can_see_eligible()
    ...
    def _can_see_eligible(self):
        ...
        for rec in self:
            ...
            if equal or self.env.user.has_group('security_groups.group_hc_employee'):
                # rec.update({'can_see_eligible': True})
                rec.can_see_eligible = True
            else:
                rec.can_see_eligible = False
                # rec.update({'can_see_eligible': False})
...
class ApprovalApprover(models.Model):
    ...
    def unlink(self):
        for rec in self:
            ...
            activities = self._get_user_approval_activities(user=rec.user_id, res_ids=rec.request_id.ids,
                                                            activity_type_id=activity)
            activities.unlink()
        # next_approvers = self.request_id.approver_ids - self
        # next_approvers = self.request_id.approver_ids
